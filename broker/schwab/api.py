...
import requests
import threading
import broker.schwab.terminal as terminal
...
def initialize(accountNumber=None):
    # load credentials
    load_dotenv()
...
def set_account_number(accountNumber):
        resp = accounts.accountNumbers()
        if resp.ok:
            d = resp.json()
            for i in range(len(d)):
                if d[i].get('accountNumber', None) == accountNumber:
                    credentials.accountNumber = d[i].get('accountNumber', None)
                    credentials.accountHash = d[i].get('hashValue', None)
                    break
            if credentials.accountNumber is None:
                terminal.colorPrint.error("Account number not found.")
                terminal.colorPrint.error("Please make sure that the account number is correct.")
        else:
            terminal.colorPrint.error("Could not get account numbers and account hash.")
            terminal.colorPrint.error(
                "Please make sure that your app status is \"Ready For Use\" and that the app key and app secret are valid.")
